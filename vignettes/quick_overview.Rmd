...
The F-value for adding the interaction term to the linear model can be used can be used as a feature that ranks orthogroups in base of how likely they are to behave differently in between species

```{r}
og_fit <- og_fit[order(og_fit$`spc:treat`, decreasing = TRUE), ]
```

This is the highest ranked orthogroup
