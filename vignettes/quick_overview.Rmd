...
We use this code to extract the annotations for orthogrups and save them in the `rowData` slot.
...
We can extract the functional informations for the top tags with:
...
We can use the function `get_top_tags()` in order to apply an arbtrary cutoff and to extract the top n differentially expressed orthogroups, in this case 50.
...
Making reliable statistical tests on cross-species transcriptomic data is a complicated and way beyond my expertise. Please, do not take reults on a low number of replicates (2, 3 or 5) as definitive. Consider validating your observations with more measurements.
