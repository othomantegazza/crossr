...
2. organizes those values in a `data.frame` using the **subfolder names** as column ids and the feature names as row ids. This `data.frame` is named **expression matrix** or **expression set**. 
...
## Load quantification from other mapping algorithms

While we suggest Salmon, you can use crossr downstream of **any mapping algorithm** but you will have to load the expression set in R by yourself. The package [tximport](http://bioconductor.org/packages/release/bioc/html/tximport.html) can help.
...
In the expression set every column represents an RNAseq sample, so now you might want to load in R also a table of metadata that describes the **experimental condition** under which every sample was harvested.
...
In R the `data.frame` that stores sample metadata is generally named `coldata`, every row of `coldata` matches one RNAseq sample (and has to match one column of the expression matrix) and every column of `coldata´ stores information on an experimental variable.

In this example we use this [sample metadata](https://www.ncbi.nlm.nih.gov/Traces/study/?acc=SRP058527&go=go) set. We do not provide dedicated function for loading metadata into R because. To do so we can use `read.table` or any similar function. 
...
Please, apply anything you know about [tidying datasets](https://cran.r-project.org/web/packages/tidyr/vignettes/tidy-data.html) to make the `coldata` more direct and easy to use.
...
- **set the sample info as column name of both datasets instead of the SRR id**,
...
- **This is what our tidy `coldata` look like, with information on SRR id, organinsm and heat shock timing:**

```{r}
coldata_tidy <- sample_info[, c("Sample_Name_s", "Organism_s", "etime")]
