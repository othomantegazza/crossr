...
This paper and dataset provide matched RNA-seq timepoints for heat shock and recovery of *Arabidopsis thaliana* and *Arabidopsis lyrata* samples, which are useful for illustrating the crossr workflow.
...
list.files(thaliana_path)
list.files(thaliana_path, recursive = T)
```

The function `make_TPM_df` extracts the TPM from all the `quant.sf` files in every subfolder and organizes them in a `data.frame`.
