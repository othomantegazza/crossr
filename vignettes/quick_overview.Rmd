...
The function `make_TPM_df()` loads the Salmon output in R. It takes as input the **path** to the folder that contains **all the Salmon output subfolders** for a species

For example, the folder that contains the quantification for *A. thaliana* has this structure:
...
### Output of $ tree -A inst/exdata/thaliana
```

In detail, the function `make_TPM_df()` extracts the TPM from all the `quant.sf` files in every subfolder and organizes them in a `data.frame`, using the subfolder names as column ids. It does not matter it the subfolders contain additional files, `make_TPM_df()` will ignore them.

This code extracts TPM values and and builds the expression matrix for ***A. thaliana*** from the folder mentioned above.
...
## extract TPM and build dataframe
...
While we suggest Salmon, **crossr could take any kind of normalized gene expression values**. We suggest to use expression values such as transcript per million (TPM), that do not depend on gene length.
