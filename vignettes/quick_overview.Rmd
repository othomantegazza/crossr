...
Usually **sample metadata** are available they might be used to change the `colnames` of the expression sets from the sample ID to names that provide more immediate information. In this case we can use this [sample sheet](https://www.ncbi.nlm.nih.gov/Traces/study/?acc=SRP058527&go=go) to do so.
...
The homology groups are called orthogroups in [orthofinder](https://github.com/davidemms/OrthoFinder) are defined as:
...
We use this **bash** code in order to produce orthogroups.
...
The function `parse_orthogroups()` takes as input the path to the `OrthologousGroups.csv` that you want to parse and load in R. Please not that `parse_orthogroups()` only of parsing **exclusively the csv file**.
...
Since orthogroups are inferred on protein sequences but short reads are mapped on transcript sequences you might have to convert the **protein ID** in the orthogroups in **transcript ID**. 
